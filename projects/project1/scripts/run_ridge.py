...
def load_csv_data(data_path, sub_sample=False, ratio=0.02, seed=7):
    """Loads data """
    """Function arguments: data_path (string name of path of .csv input file),
                           sub_sample (boolean flag that determines whether to select a subsample), 
                           ratio (defines the ratio of a subsample to the entire sample), 
...
def predict_labels(weights, data, is_LR=False):
    ...
    """Arguments: weights (weight coefficients),
                  data (features),
                  is_LR (boolean flag that determines whether the logistic regression (is_LR = true) or least squares (is_LR = false) model is used)
    """
...
def standardize(x):
    """Standardize data set by subtracting the mean and dividing result by corresponding standard deviation"""
...
def correction_missing_values(tX):
    """Correction missing values by median of feature (without taking into account the missing values for calculating the median of feature)"""
    tX[tX==-999] = float('nan')
...
def correction_missing_values_test(tX_test, col_median_tX):
    """Correction missing values in testing set using measurements (medians of corresponding features) of training set"""
...
def remove_outliers(tX):
    """Removing outliers using interquartile range and replacing them by median of feature 
    (without taking into account the missing values for calculating the median of feature)"""
...
def remove_outliers_test(tX_test, perc_25, perc_75, col_median_tX):
    """Removing outliers in testing set using measurements (25th and 75th percentiles and medians of corresponding features) of training set"""
    distance = perc_75 - perc_25
...
def build_poly(x, degree):
    """polynomial basis functions for input data x (may contain several features) for j=1 (without adding a feature vector from 1) up to j=degree."""
...
def split_data(x, y, ratio, seed=1):
    ...
    """
    #permutation and selection of indices
    np.random.seed(seed)
    num_row = len(y)
...
def cross_validation_ridge_regression(y, x, k_indices, k, lambda_, degree):
    ...
    #adding a feature vector from 1 to training set to account for offset
    ...
    #adding a feature vector from 1 to testing set to account for offset
...
def cross_validation_visualization(parameter, x_label, rmse_tr, rmse_te):
    """Visualization the curves of rmse_tr and rmse_te for cross validation."""
    plt.semilogx(parameter, rmse_tr, marker=".", color='b', label='train error')
    plt.semilogx(parameter, rmse_te, marker=".", color='r', label='test error')
    plt.xlabel(x_label)
    plt.ylabel("rmse")
...
def best_degree_selection_ridge_regression(degrees, lambdas, k_fold, seed = 1):
    """cross validation for selecting the best degree of a polynomial"""
...
def best_lambda_selection_ridge_regression(degree, lambdas, k_fold, seed = 1):
    ...
    #split data in k fold
    k_indices = build_k_indices(y, k_fold, seed)
    #define lists to store the losses of training set and test set
    rmse_tr = []
    rmse_te = []
    #cross validation:
    for lambda_ in lambdas:
    ...
    return lambdas[ind_best_lambda]
...
"""cross-validation for selection the best degree of a polynomial in ridge regression (it takes about 15 minutes)"""
#k_fold = 4
#degree = best_degree_selection_ridge_regression(np.arange(1,11), np.logspace(-10, 0, 30), k_fold)
#print("Best polynomial degree for ridge regression = {deg}".format(deg=degree))

"""cross-validation for selection the best lambda in ridge regression (it takes about 2 minutes)"""
#lambda_ = best_lambda_selection_ridge_regression(degree, np.logspace(-10, 0, 50), k_fold)
#print("Best lambda for ridge regression = {lam}".format(lam=lambda_))


#parameters that were defined by cross-validation
degree = 4
...
print("training loss (mse) = {l_tr}".format(l_tr=compute_mse(y_training, tx_training, w_ridge)))
print("testing loss (mse) = {l_te}".format(l_te=compute_mse(y_testing, tx_testing, w_ridge)))
print("Accuracy of ridge regression: "+str(round(100*np.sum(predict_labels(w_ridge, tx_testing, False)==y_testing)/len(y_testing),5))+" %\n")
...
#preprocessing test data (the same actions as for the training set but using measurements obtained from the training set)
